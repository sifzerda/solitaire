...
const Solitaire = () => {
  ...
  const onCardClick = (pileIndex, cardIndex) => {
    setTableau(prevTableau => {
      const updatedTableau = [...prevTableau];
      updatedTableau[pileIndex] = {
        ...updatedTableau[pileIndex],
        faceUp: updatedTableau[pileIndex].faceUp.map((val, idx) => idx === cardIndex ? true : val)
      };
                      ...
                      {pile.cards.map((card, index) => (
                        <>
                          {pile.faceUp[index] ? (
                            ...
                            <div
                              className={`tableau-card facedown`}
